...
async function balanceF(cookie: string): Promise<number> {
  const res = await get('/api/points/balances', cookie)
...
describe('order lifecycle', () => {
  it('creates an order, enforces the 5-pending cap', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const res = await post('/api/orders', { note: `order ${i}` }, a.cookie)
      expect(res.status).toBe(201)
    }
    const sixth = await post('/api/orders', {}, a.cookie)
  ...
  it('SUPER_ADMIN cannot create orders (403)', async () => {
    const admin = await seedAdmin()
    const res = await post('/api/orders', { note: 'x' }, admin.cookie)
  ...
  it('approve pays +50 creator / +10 referrer to the F wallet', async () => {
    ...
    const create = await post('/api/orders', { note: 'went abroad' }, b.cookie)
    const { order } = await create.json<{ order: { id: string } }>()

    const approve = await post(`/api/admin/orders/${order.id}/approve`, undefined, admin.cookie)
    expect(approve.status).toBe(200)
    expect((await approve.json<{ order: { status: string } }>()).order.status).toBe('APPROVED')

    expect(await balanceF(b.cookie)).toBe(60) // 10 + 50
  ...
  it('double-approve is a 409 with no extra ledger rows', async () => {
    ...
    const { order } = await (await post('/api/orders', {}, b.cookie)).json<{ order: { id: string } }>()

    await post(`/api/admin/orders/${order.id}/approve`, undefined, admin.cookie)
    const countAfterFirst = await ledgerCount()

    const second = await post(`/api/admin/orders/${order.id}/approve`, undefined, admin.cookie)
  ...
  it('reject pays nothing, and approve-after-reject is a 409', async () => {
    ...
    const reject = await post(`/api/admin/orders/${order.id}/reject`, undefined, admin.cookie)
    expect(reject.status).toBe(200)
    expect(await balanceF(b.cookie)).toBe(10) // registration only, no reward
  ...
  it('IDOR: a user fetching another user\'s order gets 404, not 403', async () => {
    ...
    const res = await get(`/api/orders/${order.id}`, b.cookie)
