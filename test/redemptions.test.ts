...
async function balanceF(cookie: string): Promise<number> {
  return (await (await get('/api/points/balances', cookie)).json<{ f: number }>()).f
}
...
async function unlockedUser(adminCookie: string, adminRef: string, phone: string): Promise<RegisteredUser> {
  ...
  const { order } = await (await post('/api/orders', {}, u.cookie)).json<{ order: { id: string } }>()
...
describe('redemption', () => {
  ...
  it('deducts exactly and leaves the remainder', async () => {
    ...
    expect(await balanceF(b.cookie)).toBe(20)
  })

  it('rejects an over-balance redemption (422) and writes nothing', async () => {
    const admin = await seedAdmin()
    const b = await unlockedUser(admin.cookie, admin.referralCode, '0912345678')
  ...
  it('a replayed idempotencyKey is a 409 with no extra rows', async () => {
    ...
    const first = await post('/api/admin/redemptions', { userId: b.id, f: 10, idempotencyKey: key }, admin.cookie)
    expect(first.status).toBe(201)
    const after = await ledgerCount()

    const replay = await post('/api/admin/redemptions', { userId: b.id, f: 10, idempotencyKey: key }, admin.cookie)
  ...
  it('draining: consecutive redemptions succeed until the balance runs out', async () => {
    ...
    const first = await post('/api/admin/redemptions', { userId: b.id, f: 60, idempotencyKey: crypto.randomUUID() }, admin.cookie)
    expect(first.status).toBe(201)
    expect(await balanceF(b.cookie)).toBe(0)

    const beyond = await post('/api/admin/redemptions', { userId: b.id, f: 1, idempotencyKey: crypto.randomUUID() }, admin.cookie)
