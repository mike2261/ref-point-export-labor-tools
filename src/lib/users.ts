...
export async function listUsers(db: D1Database, page: number, limit: number): Promise<{ users: AuthUser[]; total: number }> {
  const offset = (page - 1) * limit
  const [rows, count] = await Promise.all([
    db.prepare("SELECT * FROM users WHERE role = 'USER' ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(limit, offset).all<UserRow>(),
    db.prepare("SELECT COUNT(*) AS total FROM users WHERE role = 'USER'").first<{ total: number }>(),
  ])
  return { users: rows.results.map(toAuthUser), total: count?.total ?? 0 }
}
